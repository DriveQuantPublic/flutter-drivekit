...
enum class PigeonOccupantRole(val raw: Int) {
  DRIVER(0),
  PASSENGER(1),
  UNAVAILABLE(2),
  NOT_APPLICABLE(3);
  ...
  companion object {
    fun ofRaw(raw: Int): PigeonOccupantRole? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
...
data class PigeonBeaconData (
  ...
  companion object {
    @Suppress("LocalVariableName")
    fun fromList(__pigeon_list: List<Any?>): PigeonBeaconData {
      val proximityUuid = __pigeon_list[0] as String
      val major = __pigeon_list[1].let { num -> if (num is Int) num.toLong() else num as Long }
      val minor = __pigeon_list[2].let { num -> if (num is Int) num.toLong() else num as Long }
      return PigeonBeaconData(proximityUuid, major, minor)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
/** Generated class from Pigeon that represents data sent in messages. */
data class PigeonOccupantInfo (
  val role: PigeonOccupantRole,
  val passengerProbability: Long

) {
  companion object {
    @Suppress("LocalVariableName")
    fun fromList(__pigeon_list: List<Any?>): PigeonOccupantInfo {
      val role = __pigeon_list[0] as PigeonOccupantRole
      val passengerProbability = __pigeon_list[1].let { num -> if (num is Int) num.toLong() else num as Long }
      return PigeonOccupantInfo(role, passengerProbability)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
data class PigeonTrip (
  ...
  val occupantInfo: PigeonOccupantInfo? = null,
...
private object TripAnalysisApiPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      147.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
          PigeonOccupantInfo.fromList(it)
        }
      }
      148.toByte() -> {
        return (readValue(buffer) as? List<Any?>)?.let {
  ...
  fun beaconDetected(callback: (Result<Unit>) -> Unit)
{
  ...
  fun beaconConfirmed(beaconArg: PigeonBeaconData, callback: (Result<Unit>) -> Unit)
{
    val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
    val channelName = "dev.flutter.pigeon.pigeon_trip_analysis_package.FlutterTripAnalysisApi.beaconConfirmed$separatedMessageChannelSuffix"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(beaconArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(FlutterTripAnalysisError(it[0] as String, it[1] as String, it[2] as String?)))
        } else {
