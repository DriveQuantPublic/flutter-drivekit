...
| beaconDetected                    | This method is called when a beacon set in the SDK is detected. It is triggered exclusively if the trip recording has not been started yet (i.e. during the `INACTIVE` state).                                                                                                                                                                                                                                                           
| beaconConfirmed                   | Called when a beacon set in the SDK is detected and confirmed during a trip. It is triggered exclusively if the trip recording has started (i.e. during the `RUNNING` state). `BeaconData`object is described [here](https://docs.drivequant.com/trip-analysis/android/references#beacondata) |
