...
| tripStarted (depreacted)                   | Called each time a trip is started. `StartMode` indicates which event starts the trip.                                                                                                                                                                    |
...
| tripFinished (deprecated)         | Called when a trip has been recorded by the SDK and sent to DriveQuant's server to be analyzed.  `TripResponseStatus` object contains trip response status details. More about this object [here](https://docs.drivequant.com/trip-analysis/ios/references#tripresponsestatus) |
